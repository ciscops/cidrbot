...
class cidrbot:
    ...
    def send_timed_msg(self):
        id_list = self.dynamo.dynamo_db("all_ids", None, None, None, None, None)

        for room_id in id_list:
            message = self.git_handle.scan_repos("List", 'All',
                self.dynamo.dynamo_db("repos", None, None, None, None, room_id), False)
    ...
    def weekly_reminder_email(self):
        remind_users = self.dynamo.dynamo_db('notif_users', None, None, None, None, None)
        self.logging.debug(remind_users)
        messaged_users = []

        for room in remind_users['Items']:
            assigned_issues_dict = self.git_handle.scan_repos(
                "Dict", 'All', self.dynamo.dynamo_db("repos", None, None, None, None, room['room_id']), False)
    ...
    def webhook_request(self, event):
        ...
        if event_type == "New user":
            ...
            if user_id == self.webex_bot_id:
                ...
                self.dynamo.dynamo_db('delete_room', webex_msg_sender, None, None, None, self.roomID)
                return
            ...
            if member_count == 1:
                ...
                self.dynamo.dynamo_db('delete_user', webex_msg_sender, None, None, None, self.roomID)
