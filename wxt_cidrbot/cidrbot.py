...
class cidrbot:
    ...
    repo = git_init.get_repo("ciscops/cidrbot") # Cycle through the different repos, make this work through an env var
    ...
    if repo.open_issues > 0: # Easy way to differetiate between issues and no issues
        for issue in open_issues: # Iterate through the given issues
            ...
            for i in Api.people.list(git_email): # Easiest way to do this, its a generator container so this will always look ugly
                i = i.to_dict()
